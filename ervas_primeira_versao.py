...
def intersecao_sequencial(directory_2, talhoes_path):
    # Listar todos os shapefiles no diretório dissolve_mensal
    shapefiles = sorted([f for f in os.listdir(directory_2) if f.endswith('.shp')])
    ...
    # Fazer a interseção sequencial dos shapefiles
    for i in range(len(shapefiles) - 1):
        file1 = os.path.join(directory_2, shapefiles[i])
        file2 = os.path.join(directory_2, shapefiles[i + 1])
        ...
        # Nomear o shapefile resultante de acordo com a sequência
        interseccao_path = os.path.join(directory_2, f"intersect_{i+1}_{i+2}.shp")
        interseccao.to_file(interseccao_path)
        intersecoes_sequenciais.append(interseccao)
        #print(f"Interseção sequencial salva: {interseccao_path}")
    ...
    for i, interseccao in enumerate(intersecoes_sequenciais):
        ...
        # Exportar o DataFrame para um arquivo Excel
        caminho_saida = directory+'/saida/planilhas/ervas_recorrentes_mensal_acumulado_fazendas.xlsx'  # Substitua pelo caminho desejado
        df_concatenado.to_excel(caminho_saida, index=False)


directory_2 = directory+'/saida//ervas_acumuladas'

saida_intersect = directory_2+'/saida/intersect_final_acumulado/'
if not os.path.exists(saida_intersect):
    os.makedirs(saida_intersect)
print('\nPARTE 4: Exportando Recorrencias Mensais Acumuladas')
intersecao_sequencial(directory_2, talhoes_path)
print('\nFinalizado!')
